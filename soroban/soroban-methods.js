...
const RPC_URL = {
  PUBLIC: `https://rpc.ankr.com/stellar_soroban/${process.env.ANKR_KEY}`,
  TESTNET: `https://rpc.ankr.com/stellar_testnet_soroban/${process.env.ANKR_KEY}`,
};
...
async function getVersionData(contractId, network = "PUBLIC") {
  try {
    const server = new rpc.Server(RPC_URL[network]);
    const source = await server.getAccount(internalSigner.publicKey());

...
const { contractGet, getSymbol } = require("../soroban/soroban-methods");
...
const tokenSchema = new Schema(
  ...
  { _id: false } // prevents extra _id for each token
);
...
tokenListSchema.statics.addTokenToList = async function (
  ...
  if (!token) {
    throw new Error("Token must include contract");
  }

  const contract = token.trim().toUpperCase();

  const symbol = await getSymbol(contract, network);
  if (!symbol) {
    throw new Error("Token must include symbol");
  }
  ...
  await this.updateOne(
    query,
    { $setOnInsert: { userId: query.userId } },
    { upsert: true }
  );

  await this.updateOne(query, { $pull: { [`tokens.${net}`]: { contract } } });

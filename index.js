...
const discordLinkRoutes = require("./src/routes/discordMapInvoke.routes");
const telegramLinkRoutes = require("./src/routes/telegramMapInvoke.routes");
const emailLinkRoutes = require("./src/routes/emailMapInvoke.routes");
...
const { bestUsdQuoteSingle } = require("./soroban/price-computation");
...
app.use("/auth", discordLinkRoutes);
app.use("/auth", telegramLinkRoutes);
app.use("/auth", emailLinkRoutes);
...
app.post("/init-passkey-login", async (req, res) => {
	try {
		...
		console.error("INIT PASSKEY LOGIN ERROR:", error);
		res.status(500).json({
...
app.post("/init-passkey-register", async (req, res) => {
	try {
		const { username = null, network = "TESTNET" } = req.body;
		...
		console.error("INIT PASSKEY REGISTER ERROR:", error);
		res.status(500).json({
...
app.post("/verify-email", async (req, res) => {
	const { email, otp, network } = req.body;

	if (!email || !otp || !network) {
		return res
			.status(400)
			.json({ error: "email, otp and network are required" });
	}
	...
	const wallet = user?.address?.[network];
	...
	const initData = await initSignConnectTransactionService({
		...
		signRequest: {
			...
			args: [
				{ value: verification.email, type: "scSpecTypeString" },
				{ value: "email", type: "scSpecTypeString" },
			],
			sId: "",
		},
	});
...
app.get("/init-discord-auth", async (req, res, next) => {
	const { token, network } = req.query;
	...
	req.session.discord_auth_context = { userId, network };

	req.session.save((err) => {
		if (err) {
			console.error("[init-discord-auth] Session save error:", err);
			return res
				.status(500)
				.json({ error: "Session error", detail: err.message });
		}
...
app.post("/init-telegram-link", async (req, res) => {
	...
	const alreadyLinked = await UserAccount.findOne({
		"telegram.username": normalizedUsername,
	}).lean();
	...
	await TelegramLinking.deleteMany({
		userId,
		status: { $in: ["VERIFIED", "OTP_SENT"] },
	});

	const expiresAt = new Date(Date.now() + 15 * 60 * 1000);

	const pending = await TelegramLinking.findOneAndUpdate(
...
app.post("/verify-telegram-otp", async (req, res) => {
	const { otp, network } = req.body;

	if (!otp || !network) {
		return res.status(400).json({ error: "otp and network are required" });
	}
	...
	const initData = await initSignConnectTransactionService({
		...
		signRequest: {
			...
			callFunction: "add_id_wallet_map",
			args: [
				{ value: verification.telegramChatId, type: "scSpecTypeString" },
				{ value: "telegram", type: "scSpecTypeString" },
			],
			sId: "",
		},
	});
	...
	await TelegramLinking.updateOne(
		{ _id: verification._id },
		{
			$set: {
				status: "VERIFIED",
...
app.post("/get-fee-quote", async (req, res) => {
	try {
		const { wallet, asset, amount = "0", network = "TESTNET" } = req.body;
		...
		if (feeQuote) {
			...
			const fee = normalizeDecisionAmounts(feeJson);

			payloadToSend = JSON.stringify(fee);
		}
